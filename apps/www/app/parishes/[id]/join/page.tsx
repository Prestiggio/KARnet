...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
    ...
    const url = `${base}/${locale}/parishes/${id}`;
    ...
    return {
        ...
        openGraph: {
            ...
            images: [{
                url: `${process.env.NEXT_PUBLIC_CDN_HOST}/assets/${parish.picture?.id}`,
                width: parish.picture?.width, height: parish.picture?.height, alt: parish.name,
            }],
        },
...
export default async function LoginPage({ params }: { params: Promise<{ id: string, locale: string }> }) {
    const [{ id, locale }, __] = await Promise.all([
        params,
        getTranslations()
    ])
