    ...
    if(state.sent) {
        return <form action={otpFormAction}>
            <div className="text-center space-y-4 text-slate-600 dark:text-slate-100">
                ...
                <button onClick={()=>{otpSubmitted.current = true}} disabled={otpPending} className="mt-12 capitalize bg-slate-600 dark:bg-zinc-100 font-semibold font-barlow text-lg dark:text-slate-600 text-white w-full py-3 shadow-lg disabled:opacity-50">{__(`'zay`)}</button>
            </div>
        </form>
    }
