...
/**
 * Proxy sécurisé vers Elasticsearch pour la recherche de doublon "persons".
...
export default defineEndpoint({
	...
	handler: (router, { services, database, getSchema, env, logger }) => {
		router.get('/', async (req, res) => {
			...
			// Le proxy interroge directement l'index ES "persons" (nom/prénom/date
			...
			// "persons", sous peine de laisser n'importe quel compte authentifié
			...
			try {
				...
				logger.error(err, 'Persons dedup search proxy failed');
